...
// ── Fetch cart items for this guest ──
$cartItems = $db->query("
...
foreach ($cartItems as &$item) {
    ...
    $item['addons']      = $db->query("SELECT ca.quantity, ca.subtotal, a.addon_name FROM cart_addons ca JOIN addons a ON ca.addon_id = a.addon_id WHERE ca.cart_id = $cid")->fetch_all(MYSQLI_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_payment'])) {
    ...
    if (empty($errors)) {
        ...
        // Insert one reservation + one payment_record per cart item
        foreach ($cartItems as $item) {
            ...
            // Insert reservation
            $rStmt = $db->prepare("INSERT INTO reservations (guest_id, facility_id, num_guests, checkin_date, checkout_date, rate_type, total_amount, exceed_fee, reserved_at, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')");
            $rStmt->bind_param('iiisssdds',
            ...
            $pStmt = $db->prepare("INSERT INTO payment_records (guest_id, reservation_id, total_amount, status, payment_method, payment_date, payment_time) VALUES (?, ?, ?, 'pending', ?, ?, ?)");
            $pStmt->bind_param('iidsss',
                $guest_id, $reservation_id, $total_amount,
                $payment_method, $today, $now
            );
            $pStmt->execute();
        }
...
.payment-method-option:hover { border-color: var(--green-200); }
  ...
  <form method="POST" id="payForm">
      ...
      <div style="padding:16px 20px;">
        ...
        <div style="display:flex;flex-wrap:wrap;gap:12px;margin-bottom:14px;">
          <div style="background:var(--green-50);border-radius:var(--radius-sm);padding:8px 14px;min-width:130px;">
            ...
            <p style="font-size:0.78rem;color:var(--green-dark);"><?= date('g:i A', strtotime($item['checkin_time'])) ?></p>
          </div>
          ...
          <div style="background:var(--green-50);border-radius:var(--radius-sm);padding:8px 14px;min-width:130px;">
            ...
            <p style="font-size:0.78rem;color:var(--green-dark);"><?= date('g:i A', strtotime($item['checkout_time'])) ?></p>
          </div>
        </div>
    ...
    <div class="payment-method-option" id="optCash" onclick="selectPayment('cash')">
      ...
      <div style="flex:1;">
        ...
        <p style="font-size:0.82rem;color:var(--gray-400);">Pay in cash upon arrival on your reservation date. No upfront payment needed.</p>
      </div>
    </div>

    <!-- GCash (static / coming soon) -->
    <div class="payment-method-option disabled" title="GCash payment coming soon">
      ...
      <div style="flex:1;">
        ...
        <p style="font-size:0.82rem;color:var(--gray-400);">Send payment directly to our GCash number before your check-in.</p>
      </div>
    ...
    <div style="margin-top:16px;display:flex;flex-direction:column;gap:8px;">
      <div style="display:flex;gap:8px;font-size:0.8rem;color:var(--gray-400);"><span style="color:var(--green);">✔</span> Your reservation status will be <strong>pending</strong> until approved by the resort.</div>
      <div style="display:flex;gap:8px;font-size:0.8rem;color:var(--gray-400);"><span style="color:var(--green);">✔</span> You will be notified once your booking is confirmed.</div>
