...
<div class="container page-wrap">
  ...
  <div class="two-col">
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
      ...
      ?>
      <div class="card" style="padding:0;overflow:hidden;<?= $item['facility_conflict'] ? 'border:2px solid #fca5a5;' : '' ?>">
          ...
          <div style="display:flex;flex-wrap:wrap;">
            ...
            <div style="flex:1;padding:18px;min-width:0;">
              ...
              <div class="cart-conflict-banner">
                ...
                <span><?= e($item['conflict_msg']) ?> You cannot confirm this booking until dates are changed or the other reservation is cancelled.</span>
              </div>
              <?php endif; ?>
        ...
        <div id="edit_<?= $cid ?>" style="display:none;">
          <form method="POST" id="eForm_<?= $cid ?>">
            ...
            <div class="cart-edit-panel">
              ...
              <!-- Facility conflict notice in edit panel -->
    ...
    <div>
      <div class="summary-card" style="position:sticky;top:80px;">
        ...
        <?php if ($hasConflict): ?>
        <div style="background:var(--red-light);border-radius:var(--radius-sm);padding:10px 14px;margin-bottom:16px;font-size:0.82rem;color:#991b1b;display:flex;gap:8px;">
          <i class="fa-solid fa-triangle-exclamation" style="margin-top:2px;"></i>
          <span>Some items have booking conflicts. Resolve them before confirming.</span>
        </div>
        <?php endif; ?>
        <?php foreach ($cartItems as $item): ?>
        <div class="summary-row">
          <span style="max-width:60%;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:0.84rem;">
            <?= $item['facility_conflict'] ? '⚠️ ' : '' ?><?= e($item['facility_name']) ?>
          </span>
        ...
        <?php if ($hasConflict): ?>
          <button class="btn btn-full" disabled
                  style="font-size:1rem;padding:13px;margin-top:18px;background:var(--gray-200);color:var(--gray-400);cursor:not-allowed;justify-content:center;display:flex;">
...
function eChange(cid) {
  ...
  eTP[cid].availTimer = setTimeout(() => {
    if (ci) {
      eFetchFacilityAvail(cid, ci, coDate);
      eFetchAddonAvail(cid, ci, coDate);
    }
  }, 600);
}

// ── Facility availability for edit panel ──
...
// ── Addon availability for edit panel ──
...
function eToggleAddon(cid,aid,price) {
  ...
  const chk=document.getElementById('eACheck_'+cid+'_'+aid);
  if(cb.checked){card.classList.add('active');qty.style.display='flex';chk.style.background='var(--green)';chk.style.borderColor='var(--green)';chk.innerHTML='<span style="color:#fff;font-size:0.65rem;">✓</span>';}
  else{card.classList.remove('active');qty.style.display='none';chk.style.background='';chk.style.borderColor='var(--gray-200)';chk.innerHTML='';}
  eRecalc(cid);
}
...
function toggleEdit(cid) {
  ...
  if (!open) eChange(cid); // trigger availability check on panel open
}
</script>
